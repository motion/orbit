...
export async function withDomainSigningRequestConfig(domain: string, cb: (filepath: string) => Promise<any>) {
  let tmpFile = mktmp();
...
export async function withDomainCertificateConfig(domain: string, cb: (filepath: string) => Promise<any>) {
  let tmpFile = mktmp();
