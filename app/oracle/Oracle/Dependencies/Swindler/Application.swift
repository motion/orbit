...
>: ApplicationDelegate, PropertyNotifier
    ...
    fileprivate init(axElement: ApplicationElement,
        ...
        let (attrsFetched, fulfillAttrs, rejectAttrs) = Promise<[Attribute: Any]>.pending()
    ...
    fileprivate func fetchWindows(after promise: Promise<Void>) -> Promise<Void> {
        return promise.then(on: .global()) { () -> [UIElement]? in
            ...
            try traceRequest(self.axElement, "arrayAttribute", Attribute.windows) {
    ...
    fileprivate func onWindowLevelEvent(_ notification: AXNotification,
