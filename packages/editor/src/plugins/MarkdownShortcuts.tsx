...
export function MarkdownShortcuts() {
   ...
   */
  function onSpace(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
  ...
  function onDash(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
  ...
  function onBacktick(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
    const { startBlock, selection } = value
    if (selection.isExpanded) return next()
    if (startBlock.type.match(/heading/)) return next()
