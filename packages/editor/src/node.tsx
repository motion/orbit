...
export function node(View) {
  class DecoratedNode extends React.Component {
    ...
    get isRootNode() {
      return this.props.editor.getState().document.getPath(this.props.node.key).length === 1
    }

    setData = data => {
      return this.editorStore.transform(t => t.setNodeByKey(this.props.node.key, { data }))
    }
            ...
            <UI.List
              ...
              items={[
                ...
                },
                {
                  primary: 'Row',
                  onClick: () =>
                    this.editorStore.transform(t => this.editorStore.allPlugins.row.insertRow(t)),
                },
    ...
    render({ node, editor, onFocus }, { context }) {
      ...
      const isHovered =
        this.editorStore.selection.hovered && node.key === this.editorStore.selection.hovered.key
