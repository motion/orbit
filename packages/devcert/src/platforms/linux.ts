...
export default class LinuxPlatform implements Platform {
  ...
  async addToTrustStores(certificatePath: string, options: Options = {}): Promise<void> {
    ...
    await run(`sudo cp ${ certificatePath } /usr/local/share/ca-certificates/devcert.crt`);
    ...
    if (this.isFirefoxInstalled()) {
      ...
      if (!commandExists('certutil')) {
        if (options.skipCertutilInstall) {
          ...
          await run('sudo apt install libnss3-tools');
  ...
  async addDomainToHostFileIfMissing(domain: string) {
    ...
    if (!hostsFileContents.includes(domain)) {
      await run(`echo '127.0.0.1  ${ domain }' | sudo tee -a "${ this.HOST_FILE_PATH }" > /dev/null`);
    }
  }
