...
export class MediatorServer {
  ...
  private handleMessage(data: TransportRequest) {
    ...
    if (data.type === "command") {
      ...
      if (!command)
        throw new Error(`Command ${data.command} was not found. Available commands: ${this.options.commands.map(command => command.name).join(", ")}`)
    } else {
      model = this.options.models.find(model => {
        return model.name === data.model
      })
      if (!model)
        throw new Error(`Model ${data.model} was not found. Available models: ${this.options.models.map(model => model.name).join(", ")}`)
    }
    ...
    if (!resolver) {
      if (command) {
        throw new Error(`No "${data.type}" resolver for the given ${command.name} command was found`)
      } else {
