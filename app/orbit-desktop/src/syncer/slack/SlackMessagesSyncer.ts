import { LoggerInstance } from '@mcro/logger'
import { Bit, Person, SlackBitData, SlackSettingValues } from '@mcro/models'
...
export class SlackMessagesSyncer implements IntegrationSyncer {
  ...
  async run() {
    ...
    for (let channel of activeChannels) {
      ...
      log.timer(
        `loading channel ${channel.name}(#${channel.id}) database bits`,
        { oldestMessageId },
      )
      ...
      // load messages
      log.timer(`loading channel ${channel.name}(#${channel.id}) messages`, {
        oldestMessageId,
      })
      ...
      log.timer(
        `loading channel ${channel.name}(#${channel.id}) messages`,
        loadedMessages,
      )
    ...
    await getManager().transaction(async manager => {
      ...
      if (removedBits.length > 0) await manager.delete(BitEntity, removedBits)
    })
    log.timer('saving bits in the database')
