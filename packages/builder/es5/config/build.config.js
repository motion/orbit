...
module.exports = Object.assign(config, {
  ...
  entry: {
    app: filtered([IS_DEV && require.resolve('webpack-dev-server/client') + '?/', IS_DEV && require.resolve('webpack/hot/only-dev-server'), paths.appIndexJs])
  },
  ...
  resolve: {
    ...
    mainFields: IS_DEV ? ['module', 'browser', 'main', 'es6'] : ['browser', 'module:production', 'main', 'es6'],
    extensions: ['.js', '.json'],
  ...
  plugins: filtered([new InterpolateHtmlPlugin(env.raw), new HtmlWebpackPlugin({
    inject: true,
    template: paths.appHtml
  }), new webpack.DefinePlugin(env.stringified), new CaseSensitivePathsPlugin(),
  // hmr
  IS_DEV && new webpack.HotModuleReplacementPlugin(), IS_DEV && new WatchMissingNodeModulesPlugin(paths.appNodeModules),
  // readable names
