...
function XHRWrapper(this: any, xhr) {
...
XHRWrapper.prototype.getResponseHeader = function(_name) {
...
XHRTransport.prototype.open = function(
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 2) {
      ...
      onStartCallback(status, statusText, contentType, new Headers(headers))
...
function HeadersWrapper(this: any, headers) {
...
function ConnectionEvent(this: any, type, options) {
...
export function EventSourcePolyfill(this: any, url, options?) {
...
function start(es, url, options) {
  ...
  var CurrentTransport =
    options != undefined && options.Transport != undefined ? options.Transport : getBestTransport()
  var xhr =
  ...
  var onProgress = function(textChunk) {
    if (currentState === OPEN) {
      ...
      for (var position = 0; position < chunk.length; position += 1) {
        var c: any = chunk.charCodeAt(position)
        if (state === AFTER_CR && c === '\n'.charCodeAt(0)) {
          state = FIELD_START
        } else {
          if (state === AFTER_CR) {
  ...
  var onTimeout = function() {
    ...
    if (currentState !== WAITING) {
      if (!wasActivity && abortController != undefined) {
        throwError(
          new Error('No activity within ' + heartbeatTimeout + ' milliseconds. Reconnecting.'),
        )
        abortController.abort()
        abortController = undefined
      } else {
