import { BlockQuoteIcon, BoldIcon, CodeIcon, Heading1Icon, Heading2Icon, ItalicIcon, LinkIcon, StrikethroughIcon } from 'outline-icons'
import * as React from 'react'
import { Editor } from 'slate-react'
import { withTheme } from 'styled-components'

import { Block, Mark, Theme } from '../../types'
import { Separator } from './Separator'
import { ToolbarButton } from './ToolbarButton'

type Props = {
  editor: Editor
  onCreateLink: (Event) => void
  theme: Theme
}

class FormattingToolbarComponent extends React.Component<Props> {
  /**
   ...
   * @param {String} type
   */
  onClickMark = (ev: Event, type: string) => {
    ev.preventDefault()
    ev.stopPropagation()
  ...
  onClickBlock = (ev: Event, type: string) => {
    ev.preventDefault()
    ev.stopPropagation()
    const { editor } = this.props
...
export const FormattingToolbar = withTheme(FormattingToolbarComponent)
