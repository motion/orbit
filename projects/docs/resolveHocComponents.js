...
module.exports = (ast, recast) => {
  ...
  recast.visit(ast, {
    visitCallExpression(path) {
      ...
      const property = optionsNode && optionsNode.properties.find(p => p.key.name === 'Component')

      if (property) {
        type = property.value.type === 'Identifier' ? property.value.name : property.value.raw
      }
...
${comment || ''}
export default class ${upperFirst(camelCase(prefixNode.value))} extends React.Component {
  static propTypes = {
  ...
  render() {
    return null
      ...
      components = components.concat(resolver.findExportedComponentDefinition(comp.program, recast))
      return false
    },
  })
