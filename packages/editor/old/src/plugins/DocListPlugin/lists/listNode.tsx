        ...
        <list>
          ...
          <docs if={hasLoaded && hasDocs}>
            {(listStore.docs || []).map((doc, index) => (
              <item key={doc._id} onClick={() => Router.go(doc.url())} $notFirst={index > 0}>
                <UI.Glow full scale={0.7} color={[255, 255, 255]} opacity={0.04} />
                <UI.Icon name="arrows-1_circle-right-37" $icon color={darkBlue} size={20} />
