...
for (var name in cssColorNames) {
  if (cssColorNames.hasOwnProperty(name)) {
    reverseNames[cssColorNames[name]] = name
  }
}
...
export function toRgbPercent(...args) {
  var rgba = flatten(args)
...
export function toHwb(...args) {
  var hwba = flatten(args)
  var a = ''
