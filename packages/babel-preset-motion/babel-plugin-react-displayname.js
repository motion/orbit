...
module.exports = function({ types }) {
  function setDisplayNameAfter(path, nameNodeId, displayName) {
    ...
    if (blockLevelStmnt) {
      ...
      const setDisplayNameStmn = types.expressionStatement(
...
function shouldSetDisplayNameForFuncExpr(path) {
  ...
  if (
    ...
    if (path.parentPath.node.type === 'VariableDeclarator') {
      if (
        path.parentPath.parentPath.parentPath.node.type === 'ExportNamedDeclaration' ||
        path.parentPath.parentPath.parentPath.node.type === 'Program'
      ) {
        return doesReturnJSX(
          // checks regular body falls back to if its the first arg to a cb like: React.memo(() => {}) it checks that
          path.node.body || (path.node.arguments[0] && path.node.arguments[0].body),
        )
      }
    }
  }
