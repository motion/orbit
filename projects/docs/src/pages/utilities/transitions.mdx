...
<Callout title="Smooth animations">
  If you're noticing choppy animations, and the component that's being collapsed has non-zero margin
  or padding, try wrapping the contents of your <code>{'<Collapse>'}</code> inside a node with no
  margin or padding, like the <code>{'<div>'}</code> in the example below. This will allow the
  height to be computed properly, so the animation can proceed smoothly.
</Callout>
