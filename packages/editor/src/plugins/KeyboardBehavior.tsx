...
export function KeyboardBehavior() {
  ...
  function onEnter(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
  ...
  function onTab(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
  ...
  function onBackspace(ev: React.KeyboardEvent, editor: Editor, next: Function) {
    const { value } = editor
    const { startBlock, selection } = value
    if (!startBlock) return next()
    ...
    if (selection.isCollapsed) {
      ...
      const codeMarksAtCursor = marksAtCursor.filter(mark => mark.type === 'code')
