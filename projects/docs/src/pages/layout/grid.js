...
export default withLayout(function GridSection({ data }) {
  return (
    <>
      ...
      <p>
        When no column widths are specified the <code>Col</code> component will render equal width
        columns
      </p>
      <ReactPlayground codeText={GridAutoLayout} exampleClassName={styles.example} />
      ...
      <ReactPlayground codeText={GridAutoLayoutSizing} exampleClassName={styles.example} />

      <LinkedHeading h="3" id="auto-layout-variable-sizes">
        Variable width content
      </LinkedHeading>
      <p>
        Set the column value (for any breakpoint size) to <code>"auto"</code> to size columns based
        on the natural width of their content.
      </p>
      <ReactPlayground codeText={GridAutoLayoutVariable} exampleClassName={styles.example} />
      <LinkedHeading h="2" id="responsive-grids">
        Responsive grids
      </LinkedHeading>
      <p>
        The <code>Col</code> lets you specify column widths across 5 breakpoint sizes (xs, sm, md,
        large, and xl). For every breakpoint, you can specify the amount of columns to span, or set
        the prop to <code>{'<Col lg={true} />'}</code> for auto layout widths.
      </p>
      <ReactPlayground codeText={GridResponsiveAuto} exampleClassName={styles.example} />
      <p>
        You can also mix and match breakpoints to create different grids depending on the screen
        size.
      </p>
      <ReactPlayground codeText={GridResponsive} exampleClassName={styles.example} />
      <p>
        The <code>Col</code> breakpoint props also have a more complicated <code>object</code> prop
        form: <code>{`{span: number, order: number, offset: number}`}</code> for specifying offsets
        and ordering affects.
      </p>

      <p>
        You can use the `order` property to control the <strong>visual order</strong> of your
        content.
      </p>
      <ReactPlayground codeText={GridOrdering} exampleClassName={styles.example} />
      <p>
        For offsetting grid columns you can set an `offset` value, or, for more general layout, use
        the margin class utilities.
      </p>
      <ReactPlayground codeText={GridOffsetting} exampleClassName={styles.example} />
