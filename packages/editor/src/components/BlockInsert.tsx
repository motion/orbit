...
class BlockInsertComponent extends React.Component<Props, State> {
  ...
  handleMouseMove = (ev: MouseEvent) => {
    const triggerPoint = this.ref ? this.ref.getBoundingClientRect().left + 300 : window.innerWidth
    ...
    if (result) {
      ...
      const hideToolbar =
        result.node.type !== 'paragraph' || !!result.node.text.trim() || result.node.isVoid
...
export const BlockInsert = withTheme(BlockInsertComponent)
