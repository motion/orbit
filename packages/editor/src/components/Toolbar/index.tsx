...
export class Toolbar extends React.Component<Props, State> {
  ...
  update = () => {
    ...
    newState.top = `${Math.round(rect.top + window.scrollY - this.menu.offsetHeight)}px`
    ...
    return (
      <Portal>
        <Menu active={this.state.active} ref={ref => (this.menu = ref)} style={style}>
          {this.state.link ? (
            <LinkToolbar {...this.props} link={this.state.link} onBlur={this.hideLinkToolbar} />
          ) : (
            <FormattingToolbar onCreateLink={this.showLinkToolbar} {...this.props} />
          )}
        </Menu>
      </Portal>
    )
