...
const toWords = s => s.split(" ").map(s => s.replace(/\:/g, ""))
...
export default class RelevancyStore {
  ...
  async willMount() {
    window.rel = this
    let documents = await (await fetch("/dist/marginal.json")).json()
    ...
    const includeDocs = ["Puerto Rico", "Plato"]

    this.documents = documents
      .filter(
        ({ text }) =>
          includeDocs.filter(entity => text.indexOf(entity) > -1).length > 0
      )
      .slice(0, 150)
      .map((doc, index) => ({
        ...doc,
    ...
    const str = "entities: " + this.documents.length
  ...
  idfs = () => {
    const total = {}
    this.entities.forEach((words, index) => {
      words.forEach(word => {
  ...
  @watch
  tfs = () => {
    return this.entities.map((words, index) => {
