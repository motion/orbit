...
import { defaults, pickBy, isFunction } from 'lodash'
...
function wrapVerify(slackAuthOptions) {
  return function _verify(
    ...
    slackAuthOptions.verify(accessToken, refreshToken, profile, verified)
    /*
...
class SlackStrategy extends OAuth2Strategy {
  ...
  constructor(options, verify) {
    console.log('in slack constructor')
    ...
    console.log('...merged options are', mergedOptions)
  ...
  userProfile(accessToken, done) {
    console.log('in user profile', accessToken)
    needle.request(
      'get',
