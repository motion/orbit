...
export const wordMoversDistance = ({ words, vectors }, sentence2) => {
  const list = []
  // we're going to mutate this so let's copy first

  const words2 = [...sentence2.words]
  const vecs2 = [...sentence2.vectors]
  ...
  vectors.forEach((vec, index) => {
    ...
    list.push({ val: minVal, fromWord, toWord: words2[minIndex] })
