...
import { ConfluenceSource, JiraSource, SettingSaveCommand, WebsiteSource } from '@mcro/models'
...
export const SettingSaveResolver = resolveCommand(SettingSaveCommand, async ({ setting }) => {
  log.info('saving setting', setting)
  try {
    // send test request to atlassian server to check setting credentials
    log.info('saving setting', setting)
    if (setting.type === 'jira') {
      const loader = new JiraLoader(setting as JiraSource, log)
      await loader.test()
      setting.name = extractTeamNameFromDomain((setting as JiraSource).values.credentials.domain)
    } else if (setting.type === 'confluence') {
      const loader = new ConfluenceLoader(setting as ConfluenceSource, log)
      await loader.test()
      setting.name = extractTeamNameFromDomain(
        (setting as ConfluenceSource).values.credentials.domain,
      )
    } else if (setting.type === 'website') {
      setting.name = (setting as WebsiteSource).values.url
    }
