...
See `Alternates` for a full list of alternate states, how you can define them in themes, as well as
...
Using either a `<Row />` or `<Col />` with the group prop allows for grouping, which will join
together inner elements and properly handle removing the border radius in the intermediary elements.
You may also group items using `<SegmentedRow />` to avoid wrapping them in an element.
