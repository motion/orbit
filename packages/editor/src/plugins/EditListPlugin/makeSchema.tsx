...
function listsContainOnlyItems(opts) {
  return {
    ...
    normalize(transform, node, value) {
      return value.toWrap.reduce((tr, { key }) => tr.wrapBlockByKey(key, opts.typeItem), transform)
    },
  }
}
...
function itemsDescendList(opts) {
  return {
     ...
     */
    normalize(transform, node, value) {
      return value.toUnwrap.reduce((tr, n) => tr.unwrapBlockByKey(n.key), transform)
    },
  }
}
...
function itemsContainBlocks(opts) {
  return {
    ...
    normalize(transform, node, _) {
      ...
      const wrapper = transform.state.document.getDescendant(node.key).nodes.first()
