...
export const FirstLineTitle = [
  // enforce first line as always title
  {
    ...
    validate: document => {
      ...
      const isTitle =
        firstNode && firstNode.type === BLOCKS.TITLE && firstNode.data.get('level') === 1
  ...
  // enforce other lines not title
  {
    ...
    validate: document => {
      ...
      const isTopLevelTitle = node => node.type === BLOCKS.TITLE && node.data.get('level') === 1
